...
// Model configurations - dynamically loaded from models.json
let MODEL_CONFIGS = {};
let AVAILABLE_MODELS = [];

/**
 * Load models from models.json (generated by ProxyMe plugin)
 * Falls back to default models if file doesn't exist
 */
function loadModelsConfig() {
  ...
  try {
    if (fs.existsSync(modelsConfigPath)) {
      console.log("📋 Loading models from:", modelsConfigPath);
        ...
        config.models.forEach((model) => {
          if (model.enabled) {
            MODEL_CONFIGS[model.id] = {
              ...
              description: `${model.category} model (${model.provider})`,
              temperature: model.temperature || 0.7,
              stream: model.stream !== undefined ? model.stream : true,
              customHeaders: model.customHeaders || {},
              customBodyParams: model.customBodyParams || {},
            };
          }
        });

        console.log(
          `✅ Loaded ${Object.keys(MODEL_CONFIGS).length} enabled models from ProxyMe configuration`,
        );
      }
    } else {
      console.log("⚠️  models.json not found, using default models");
      loadDefaultModels();
    }
  } catch (error) {
    console.error("❌ Error loading models.json:", error.message);
    console.log("⚠️  Falling back to default models");
    loadDefaultModels();
  }

  // Build available models list for OpenAI format
  AVAILABLE_MODELS = Object.keys(MODEL_CONFIGS).map((modelId) => ({
    id: modelId,
    object: "model",
...
app.post("/v1/chat/completions", async (req, res) => {
  ...
  try {
    ...
    const requestBody = {
      ...
      stream: stream,
      temperature: config.temperature || otherParams.temperature,
      ...otherParams,
...
const server = app.listen(PORT, () => {
  console.log("\n" + "=".repeat(60));
  console.log("🚀 Rider IDE AI Proxy Server Started (v2.1.0)");
  console.log("=".repeat(60));
  console.log(`📍 Server running on http://localhost:${PORT}`);
  console.log(`🔗 Configure Rider IDE with: http://localhost:${PORT}/v1`);
  console.log(`🐛 Debug mode: ${DEBUG ? "ENABLED" : "DISABLED"}`);
  console.log("\n📋 Available Models (from ProxyMe configuration):");
  if (Object.keys(MODEL_CONFIGS).length === 0) {
    console.log("   ⚠️  No models loaded! Check ProxyMe settings.");
  } else {
  ...
  console.log("\n💡 ProxyMe Plugin:");
  console.log(
    `   Only models enabled in ProxyMe settings will appear in Rider`,
  );
  console.log(`   Modify models in: Tools → ProxyMe → Settings`);
  console.log("=".repeat(60) + "\n");
});
